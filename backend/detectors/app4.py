...
# Khi chạy trực tiếp ``py backend/detectors/app4.py``, Python chỉ thêm thư
# mục detectors vào sys.path nên không thấy backend/function. Thêm đúng thư
# mục backend để chế độ debug trực tiếp và chế độ main.py import dùng chung.
...
def read_part1(part1_roi, rows, cols, answer_key_1, img=None, offset_x=0, offset_y=0, start_question=1):
    ...
    thresh = cv2.adaptiveThreshold(gray,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,31,13)
    ...
    for c in cnts:
        ...
        if  150 <= area < 350 and 0.9 < aspect_ratio < 1.6 and peri > 40:
            cx = x + w//2
            cy = y + h//2
            # cv2.putText(part1_roi,f"{int(area)}",(x, y-3),cv2.FONT_HERSHEY_SIMPLEX,0.4,(0,0,255),1)
...
def build_part2_grid(block_img , answer_key_2, cols=4, start_question = 1):
    ...
    if len(rows_y) < len(choices):
        raise ValueError(
            f"Phần II chỉ tìm thấy {len(rows_y)}/4 hàng đáp án"
        )

    if len(rows_y) > len(choices):
        # Mỗi hàng a/b/c/d thật có nhiều bubble cùng cao độ. Các đường chữ,
        # viền bảng hoặc nhiễu thường chỉ tạo một vài contour, vì vậy ưu tiên
        # bốn cụm có nhiều tâm bubble nhất rồi sắp lại từ trên xuống dưới.
        rows_y = sorted(
            sorted(
                rows_y,
                key=lambda row_group: len(row_group),
                reverse=True,
            )[:len(choices)],
            key=lambda row_group: float(np.mean(row_group)),
        )
...
def build_part3_grid(block_img, answer_key_3, question_no, rows=12, cols=4):
    ...
    # Hàng dấu âm chỉ có rất ít contour nên đôi lúc không được gom vào lưới.
    # Khôi phục hàng còn thiếu dựa trên khoảng cách đều giữa 12 hàng chuẩn:
    # dấu âm, dấu phẩy và các số 0-9.
    if 2 <= len(rows_y) < rows:
        row_gaps = np.diff(rows_y)
        row_spacing = int(round(float(np.median(row_gaps))))

        while len(rows_y) < rows and row_spacing > 0:
            large_gap_index = next(
                (
                    index
                    for index, gap in enumerate(np.diff(rows_y))
                    if gap > row_spacing * 1.5
                ),
                None,
            )

            if large_gap_index is not None:
                rows_y.insert(
                    large_gap_index + 1,
                    rows_y[large_gap_index] + row_spacing,
                )
            elif rows_y[0] - row_spacing >= 0:
                rows_y.insert(0, rows_y[0] - row_spacing)
            elif rows_y[-1] + row_spacing < H:
                rows_y.append(rows_y[-1] + row_spacing)
            else:
                break
    ...
    for r, cy in enumerate(rows_y):
        ...
        for cx in cols_x:
            ...
            row_ratios.append(ratio_fill)
            cv2.circle(debug, (cx, cy), radius, (0, 255, 255), 2)

        fill_ratios.append(row_ratios)

    # Dấu âm và dấu phẩy là hai hàng điều khiển riêng, không phải chữ số.
    minus = False
    comma_pos = -1

    if fill_ratios and fill_ratios[0]:
        minus_col = int(np.argmax(fill_ratios[0]))
        minus_ratio = fill_ratios[0][minus_col]

        if minus_ratio > 0.3:
            minus = True
            selected_points.append(
                (0, minus_col, cols_x[minus_col], rows_y[0], radius)
            )

    if len(fill_ratios) > 1 and fill_ratios[1]:
        comma_col = int(np.argmax(fill_ratios[1]))
        comma_ratio = fill_ratios[1][comma_col]

        if comma_ratio > 0.3:
            comma_pos = comma_col
            selected_points.append(
                (1, comma_col, cols_x[comma_col], rows_y[1], radius)
            )

    # Từ hàng số 0 trở xuống, mỗi cột chỉ được chọn đúng một ô: ô có
    # lượng tô lớn nhất. Vì vậy nhiễu không còn tạo hai chữ số trong một cột.
    for c, cx in enumerate(cols_x):
        digit_candidates = [
            (fill_ratios[r][c], r)
            for r in range(2, min(len(fill_ratios), 12))
        ]
    ...
    # Một số đề có ít hơn 8 câu trả lời ngắn. Không được làm app4 dừng
    # bằng KeyError khi ROI còn lại không có đáp án chuẩn.
    if correct_data is None:
        print(
            f"Bỏ qua câu Phần III {question_no}: "
    ...
    if isinstance(correct_data, dict):
        ...
        accepted_answers_for_display = (
            correct_data.get("acceptedAnswers")
            or [correct_answer]
        )
    else:
        correct_answer = correct_data
        accepted_answers_for_display = [correct_data]

    if correct_answer in (None, ""):
        correct_answer = next(
            (
                value
                for value in accepted_answers_for_display
        ...
        if correct_text.startswith("-"):
            if rows_y and cols_x:
                correct_points.append(
                    (cols_x[0], rows_y[0], correct_radius)
                )
...
def detect(image_path, answer_keys=None, debug_mode=False):
    ...
    missing_markers = [
        name
        for name, point in marker_names.items()
        if point is None
    ]

    if missing_markers:
        raise ValueError(
            "Không tìm đủ 4 marker của mẫu app4; thiếu: "
...
if __name__ == "__main__":
    ...
    default_answer_keys_path = os.path.join(
    ...
    answer_keys_path = (
        sys.argv[2]
        if len(sys.argv) > 2
        else default_answer_keys_path
    )

    try:
        with open(answer_keys_path, "r", encoding="utf-8") as answer_file:
            answer_data = json.load(answer_file)

        if "answerSheetTemplates" in answer_data:
            app4_templates = [
                template
                for template in answer_data["answerSheetTemplates"]
                if template.get("templateId") == "template-004"
                and template.get("answerKeys")
            ]

            if not app4_templates:
                raise ValueError(
                    "Không tìm thấy answerKeys của template-004 trong db.json"
                )

            def template_answer_count(template):
                return max(
                    (
                        len(answer_key.get("mcq", {}))
                        + len(answer_key.get("trueFalse", {}))
                        + len(answer_key.get("shortAnswer", {}))
                    )
                    for answer_key in template["answerKeys"].values()
                )

            debug_answer_keys = max(
                app4_templates,
                key=template_answer_count,
            )["answerKeys"]
        else:
            debug_answer_keys = answer_data

        # answers.json cũ của dự án có thể còn một khóa mã đề rỗng. Bỏ riêng
        # dữ liệu rác này ở chế độ debug; API thật vẫn được selector kiểm tra.
        debug_answer_keys = {
            str(code).strip(): answer_key
            for code, answer_key in debug_answer_keys.items()
            if str(code).strip()
        }

        result = detect(
            test_image_path,
            answer_keys=debug_answer_keys,
            debug_mode=True,
        )
        ...
        print(
            "Cách chạy: py backend/detectors/app4.py "
            "<duong-dan-anh> <duong-dan-json-dap-an>"
        )
