...
def find_marker_in_roi(roi, offset_x=0, offset_y=0, win_name="ROI DEBUG", debug_mode = False):
    ...
    for c in cnts:
        ...
        if 1 <= len(approx) <= 9 and extent > 0.6 :
...
def detect(image_path, answer_keys=None, debug_mode=False):
    ...
    # Khi chạy riêng file để debug, tự đọc đáp án từ answers.json.
    # Khi chạy qua FastAPI, answer_keys vẫn được truyền từ main.py như cũ.
    if answer_keys is None:
        answer_file_path = os.path.join(BASE_DIR, "answers.json")

        if not os.path.exists(answer_file_path):
            raise FileNotFoundError(
                f"Không tìm thấy file đáp án để debug: {answer_file_path}"
            )

        with open(answer_file_path, "r", encoding="utf-8") as answer_file:
            answer_keys = json.load(answer_file)

    if not isinstance(answer_keys, dict):
        raise TypeError("answer_keys phải là dictionary")
    ...
    # if debug_mode:
    #     cv2.namedWindow("KET QUA CHAM", cv2.WINDOW_NORMAL)
    #     cv2.resizeWindow("KET QUA CHAM", 800, 1000)
    #     cv2.imshow("KET QUA CHAM", warp)
...
#         r"\data3 7-2-2026\IMG_7862.png"
#     )

#     try:
#         # Không cần truyền answer_keys khi chạy riêng:
#         # detect() sẽ tự đọc answers.json trong cùng thư mục.
#         result = detect(
#             test_image_path,
#             debug_mode=True
#         )
...
if __name__ == "__main__":
    # Thư mục chứa toàn bộ ảnh muốn debug
    test_folder = (
        r"C:\Users\Admin\Downloads\Project_1\backend\data\data3 7-2-2026"
    )
    ...
    for pattern in image_patterns:
        image_files.extend(glob.glob(os.path.join( test_folder, pattern)))

    # Loại bỏ file trùng và sắp xếp theo tên
    ...
    if not os.path.isdir(test_folder):
        ...
        print("Không tìm thấy ảnh PNG/JPG/JPEG trong:",test_folder)
        ...
        for index, image_path in enumerate( image_files,start=1):
            ...
            try:
                # Không truyền answer_keys:
                # detect() tự đọc answers.json
                result = detect(image_path, debug_mode=True)

                print("\n====== KẾT QUẢ MODEL ======" )
                print(json.dumps(result, ensure_ascii=False, indent=4))
                all_results.append({"fileName": file_name, "success": True,"data": result,})
                print( "\nNhấn phím bất kỳ trên " "cửa sổ ảnh để xem ảnh tiếp theo.")
                ...
                print( f"Lỗi xử lý ảnh {file_name}:",  str(error))
