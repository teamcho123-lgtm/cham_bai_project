...
start "API cham bai :8001" cmd /k "cd /d "%~dp0backend" && set PYTHONIOENCODING=utf-8 && .venv\Scripts\uvicorn.exe main:app --reload --host 0.0.0.0 --port 8001"

REM ---------- 3. Giao dien : cong 3000 ----------
start "Giao dien :3000" cmd /k "cd /d "%~dp0frontend" && npm run dev"

REM ---------- 4. HTTPS camera cho iPhone ----------
start "HTTPS camera iPhone" powershell -NoExit -ExecutionPolicy Bypass -File "%~dp0start-camera-tunnel.ps1"

echo [1] API du lieu    http://localhost:8000
echo [2] API cham bai   http://127.0.0.1:8001
echo [3] Giao dien      http://localhost:3000
echo [4] HTTPS camera   Cloudflare Quick Tunnel
echo [5] iPhone         quet ma QR trong man hinh them anh
echo.
